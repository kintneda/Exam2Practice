...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        if len(self.contents) > volume:
            self.contents = ""
        if len(self.original_contents) > volume:
    ...
    def append_string(self, additional_contents):
        ...
        val = self.volume - len(self.contents) - len(additional_contents)

        if val < 0:
            self.contents = self.contents + additional_contents[:val]
            return additional_contents[val:]
        else:
    ...
    def double(self):
        ...
        val = self.volume - len(self.contents) - len(self.contents)
        double = self.contents + self.contents

        if val < 0:
            self.contents = self.contents + self.contents[:val]
            return double[val:]
        else:
    ...
    def shrink(self, new_volume):
        ...
        val = self.volume - len(self.contents)
    ...
    def double_then_shrink(self, new_volume):
        ...
        double_extra = Box.double(self)
        shrink_extra = Box.shrink(self, new_volume)

        return len(double_extra) + len(shrink_extra)
    ...
    def reset(self):
        ...
        self.historyStr = self.contents
        self.history.append(self.historyStr)
    ...
    def steal(self, other_box):
        ...
        other_box.contents = Box.append_string(self, other_box.contents)
